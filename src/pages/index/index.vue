...
const bannerList = ['https://cdn.juesedao.cn/mdy/a064a93175144fd789d3a628977a2d54', 'https://cdn.juesedao.cn/mdy/6d62ae3a3fec4e39933010403d456b54', 'https://cdn.juesedao.cn/mdy/2a81d7f356fa44d981e77ecc77220a67']
</script>
...
<template>
  <view
    ...
    style="background: linear-gradient(180deg, #FC7C44 10.05%, #F7DED2 24.44%, #F6F6F6 100.12%);z-index: -3"
  >
    <view class="px-12px">
      <!--    TopBar -->
      ...
      <!--      Valuable Card -->
      <view class="mt-32px">
        <view class="underline-gradient relative ml-25px inline-block text-18px font-bold">
          我们的价值
        </view>
        <view
        ...
        >
          <!--          内容轮播 -->
          <view style="position:relative;" class="h-full w-180px py-20px pl-24px">
            <swiper
              :current="current"
              autoplay
              :interval="3000"
            ...
            <sar-swiper-dot
              :list="list"
              field="title" :current="current" type="dot-bar"
            />
          </view>

          <image
            class="bottom-0 h-225px w-150px -right-12px" style="position:absolute;"
            :src="imgRes.standingMouse"
          />
        </view>
      </view>
      <!--      Card -->
      <view class="mt-35px flex items-center justify-between pb-150px">
        <image v-for="url in bannerList" :key="url" class="h-[240px] w-[109px] shrink-0" :src="url" mode="aspectFill" />
      </view>
    </view>
  </view>
  <content-popup v-model="popupShow" />
  <image
    v-if="!isLogin" class="fixed bottom-100px right-0 h-40px w-100px" mode="widthFix"
  ...
  >
    ...
    <view class="ml-6px text-25px text-white">
      接单中心
    </view>
    <view class="ml-auto flex flex-col">
      ...
      <view class="text-right text-10px text-white">
        马上查看
      </view>
    </view>
  </view>
